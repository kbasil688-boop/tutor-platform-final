...
export default function Dashboard() {
  ...
  const refreshData = async () => {
    ...
    // 3. FETCH DATA MANUALLY (The No-Fail Method)
    if (profileData?.is_tutor) {
      ...
      if (tData) {
        ...
        if (bData && bData.length > 0) {
           // Manually get the student names for these bookings
      ...
      if (bData && bData.length > 0) {
        // Manually get the tutor details
        ...
        // Merge everything (Booking + Tutor Info + Tutor Name)
        fetchedBookings = bData.map(booking => {
  ...
  // Tutor: Upload Lesson Logic
  const handleUploadClick = () => {
    const title = prompt("Enter Lesson Title:");
    const url = prompt("Enter YouTube Link:");
    if (title && url) {
       uploadLesson(title, url);
    }
  };

  const uploadLesson = async (title: string, url: string) => {
    const { error } = await supabase.from('lessons').insert([
        { tutor_id: tutorData.user_id, title: title, video_url: url } // Use user_id for lessons link
    ]);
        ...
        <div className="mb-10">
            {profile?.is_tutor ? (
                <button 
                  ...
                  className="w-full md:w-auto bg-blue-600 hover:bg-blue-500 text-white font-bold py-4 px-8 rounded-xl flex items-center justify-center gap-2 transition"
                >
                  <PlusCircle size={20} />
                  Upload New Lesson
                </button>
                ...
                <button 
                  ...
                  className="w-full bg-yellow-400 hover:bg-yellow-300 text-black font-bold py-4 px-8 rounded-xl flex items-center justify-center gap-2 transition shadow-[0_0_20px_rgba(250,204,21,0.3)] hover:scale-[1.02]"
                >
                  <Search size={24} />
                  FIND A TUTOR NOW
                </button>
        ...
        <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700">
          ...
          <div className="space-y-4">
            {bookings.length === 0 ? (
             ...
             bookings
                  .filter(b => b.status !== 'rejected') // <--- THIS LINE HIDES REJECTED BOOKINGS
                 .map((booking) => (
                   ...
                   <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                     ...
                     {profile?.is_tutor && booking.status === 'pending' && (
                        <div className="flex gap-2">
                          <button onClick={() => handleBookingAction(booking.id, 'confirmed')} className="bg-green-600 hover:bg-green-500 text-white p-2 rounded-lg transition" title="Accept"><Check size={20} /></button>
                          <button onClick={() => handleBookingAction(booking.id, 'rejected')} className="bg-red-600 hover:bg-red-500 text-white p-2 rounded-lg transition" title="Reject"><XIcon size={20} /></button>
                        </div>
                     )}
                   </div>
                </div>
